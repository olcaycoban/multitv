...
async function findLiveVideoByChannelId(channelId, retries = 3) {
...
async function checkAndUpdate() {
  const channels = [...getAll('main'), ...getAll('bolge')];
    ...
    if (ch.type === 'youtube') {
      ...
      if (!channelId) {
        ...
        if (channelId) {
          ...
          update(ch.id, { yt_channel_id: channelId });
...
module.exports = { checkAndUpdate, findLiveVideoByChannelId };
