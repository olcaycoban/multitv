...
// Vercel Fluid compute'ta TCP + connection pooling önerilen yöntem:
...
try {
  // Sadece Vercel ortamında gerçek işlev görür; local'de no-op'a düşer.
  attachDatabasePool = require('@vercel/functions').attachDatabasePool;
} catch { /* paket yoksa sorun değil, no-op kullanılır */ }
...
module.exports = function createDb(connectionString) {
  ...
  return { getAll, insert, update, remove, reorder, addJobLog, getJobLogs };
};
