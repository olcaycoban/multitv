...
const DB_PATH = path.join(__dirname, 'channels.db');
...
const defaultChannels = [
  ...
  { name: 'HT Spor',      source: 'gcWaPe_LBMc',                                                 type: 'youtube', screen: 'main' },
...
if (count.cnt === 0) {
  const insert = db.prepare(
    'INSERT INTO channels (name, source, type, position, screen) VALUES (?, ?, ?, ?, ?)'
  );
  const insertMany = db.transaction((channels) => {
    channels.forEach((ch, i) => {
      insert.run(ch.name, ch.source, ch.type, i, ch.screen);
    });
  });
...
function getAll(screen) {
  return db
    .prepare('SELECT * FROM channels WHERE screen = ? ORDER BY position ASC')
    .all(screen || 'main');
}

function insert(channel) {
  const maxPos = db
    .prepare('SELECT COALESCE(MAX(position), -1) as m FROM channels WHERE screen = ?')
    .get(channel.screen || 'main').m;
  const result = db
    .prepare('INSERT INTO channels (name, source, type, position, screen) VALUES (?, ?, ?, ?, ?)')
    .run(channel.name, channel.source, channel.type || 'youtube', maxPos + 1, channel.screen || 'main');
...
module.exports = { getAll, insert, update, remove, reorder };
